...
test("retries on failure and succeeds later", async () => {
  ...
  const result = await retryFetchApiRequest("url", 5, 0, fn);
...
test("returns failure after all retries", async () => {
  ...
  const result = await retryFetchApiRequest("url", 2, 0, fn);
